...
class Tod4090:
    ...
    def __init__(self):
        ...
        if not header:
            Tod4090.writer.writerow(self.fieldnames)
    ...
    def parase(length, value):
        ...
        SYNC_REPLY到达时间平均值 = (SYNC_REPLY到达时间平均值_HI * 256.0 + SYNC_REPLY到达时间平均值_LO).astype(np.uint16)

        ret = [N * [rolling], N * [anchor], tag_id0.tolist(), SYNC_REPLY到达时间平均值.tolist(), SYNC到达相位.tolist(), REPLY到达相位.tolist()]
        ret = list(zip(*ret))
        # ret = {
        #     "rolling": N * [rolling],
        #     "AnchorId": N * [anchor],
