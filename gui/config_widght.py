...
class ConfigWidght(QWidget):
    def __init__(self, uwb: Uwb):
        ...
        self.serial_widght = SerialConfig()
        self.serial_widght.setVisible(False)
        self.main_box.addWidget(self.serial_widght)
        self.udp_widght = UdpConfig()
        self.udp_widght.setVisible(False)
        self.main_box.addWidget(self.udp_widght)

        # 选择第一个RadioButton为默认选中
        if config['access_type'] == 'udp':
            self.radio_udp.setChecked(True)
            self.udp_widght.setVisible(True)
            self.radio_udp.setText('udp（当前）')
        elif config['access_type'] == 'serial':
            self.radio_serial.setChecked(True)
            self.serial_widght.setVisible(True)
    ...
    def handle_update_config(self):
        if self.radio_serial.isChecked():
            if self.serial_widght.update_serial(self.uwb.reset_access):
                self.radio_serial.setText("串口（当前）")
                self.radio_udp.setText('udp')
        elif self.radio_udp.isChecked():
            if self.udp_widght.update_port(self.uwb.reset_access):
