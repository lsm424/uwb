...
class Tof2011Widght(QWidget):
    ...
    def __init__(self):
        ...
        self.plot_fpl = self.pw2.plot(self.x_rolling, self.y_fpl, pen=pg.mkPen(color=(100, 0, 0), width=3),
                                      symbol='o', symbolBrush='r', name='fpl', symbolSize=7)
        self.plot_rxl = self.pw2.plot(self.x_rolling, self.y_rxl, pen=pg.mkPen(color=(0, 0, 100), width=3),
                                      symbol='o', symbolBrush='b', name='rxl', symbolSize=7)
        # 创建一个图例
    ...
    def tagid_selection_changed(self, index):
        try:
            ...
            if len(anchorid_list) == 0:
                logger.warning(
                    f'tof tagid切换{cur_tag_id}时，筛选不到anchorid，准备重新选择tagid')
            ...
            self.cur_anchor_id = cur_anchor_id if cur_anchor_id else {
                anchorid_list[0]}
            self.anchor_id_combox.blockSignals(True)
            self.anchor_id_combox.clear()
            self.anchor_id_combox.addCheckableItems(
                list(map(lambda x: str(x), set(anchorid_list))))
            self.anchor_id_combox.select_items(
                list(map(lambda x: str(x), self.cur_anchor_id)))
    ...
    def anchorid_selection_changed(self, index):
        ...
        self.cur_anchor_id = set(
            map(lambda x: int(x), self.anchor_id_combox.checkedItems()))
        # # 更新gui显示数据
        ...
        for x in self.cur_anchor_id:
            ...
            if 'line' not in self.plot_distance.get(x, {}):
                ploter = self.pw1.plot(x_rolling, y_distance, pen=pg.mkPen(color=random.randint(0, 0xffffff) | 0xff00, width=3),
                                       symbol='o', symbolBrush='b', name=str(x), symbolSize=7)
                logger.warning(f'生成曲线 {x} {self.plot_distance}')
            else:
        ...
        if not incr or not self.legend.items:
            ...
            for item in self.pw1.items():
                if isinstance(item, pg.PlotDataItem) and int(item.opts['name']) not in self.plot_distance:
                    ...
                    # legend.addItem(item, name=name)
            logger.warning(f'全量刷新tof，图例数量：{len(self.legend.items)} anchcor_id: {self.cur_anchor_id}, plot_distance: {self.plot_distance}')
    ...
    def recive_gui_data_thread(self):
        ...
        while True:
            ...
            if self.cur_tag_id is None or time.time() - self.last_update_tagid_time > 5:
                ...
                logger.info(
                    f'tof显示数据队列积压：{gui_queue.empty()}, 积压全量gui数据：{len(self.gui_data)}，x轴最小值{self.x_rolling[0] if len(self.x_rolling) > 0 else None}，x轴长度：{len(self.x_rolling)}，tagid数量：{len(tag_id_set)}')
